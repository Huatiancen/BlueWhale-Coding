...
def test_frontend_uses_safe_dom_rendering_and_modular_state() -> None:
    ...
    assert 'addEventListener("compositionstart"' in scripts["app.js"]
    assert 'addEventListener("compositionend"' in scripts["app.js"]
    assert 'from "./diff-view.js"' in scripts["artifact-view.js"]
    assert "changeset-card" in scripts["render.js"]
    assert '"任务已完成"' not in scripts["render.js"]
    assert '"这一步没有完成"' not in scripts["render.js"]
    assert "errorMessage(" not in scripts["render.js"]
    assert "resultStrip(" not in scripts["render.js"]
    ...
    assert '"已撤销"' in scripts["render.js"]
    assert "onUndoChangeset" in scripts["render.js"]
    assert "undoChangeset" in scripts["api.js"]
    assert "/undo`" in scripts["api.js"]
